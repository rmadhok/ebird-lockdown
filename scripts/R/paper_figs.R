...
stats <- ebird20 %>%
  ...
  summarize(`A. Num. Active Users` = n_distinct(observer_id),
            ...
            `C. Num. Species` = n_distinct(taxonomic_order)) %>%
...
ggplot(data = stats, 
  ...
  theme(panel.grid.minor = element_blank(),
        ...
        axis.line = element_blank(), 
        axis.ticks = element_blank())
ggsave('./figs/hour_distbn.png', height=4,width=8)
...
ggplot(dd_type[dd_type$var == 'Num. Species/Trip',], aes(x=(dif), y=value, group=key)) +
  ...
  labs(y='Difference in 2020 Sp. Richness\n per Trip Rel. to 2019\n',
...
ggplot(dd_type[dd_type$var == 'Num. Species/Trip',], aes(x=(dif), y=value, group=key)) +
  geom_line(aes(color=key), size=1.5) + 
  labs(y='Difference in 2019 Sp. Richness\n per Trip Rel. to 2018\n',
       x='\nDays since 4th Wednesday in March', color = 'Trip Protocol') +
  geom_vline(xintercept=0, linetype='dashed') +
  geom_hline(yintercept=0, linetype='dashed') +
  scale_colour_viridis_d() +
  ...
  theme(panel.grid.minor = element_blank(), 
        axis.line = element_blank(),
        text = element_text(size=25))
ggsave('./figs/dd_sr_placebo_t20.png', height=6,width=15)
...
coef_plot <- ggplot(estimates, aes(x = spec_no, y = est)) + 
  geom_linerange(aes(ymin = ci_l, ymax = ci_h), size = 1, alpha = 0.8) + 
  geom_point(fill = "white", shape = 21) + 
  labs(y = "Impact of Lockdown on \n Sp. Richness per Trip") + 
  geom_hline(yintercept=0, linetype='dashed') +
...
dd_plot_2 <- ggplot(dd_time, aes(x=term, y=estimate, group=1)) + 
  ...
  geom_hline(yintercept=0, linetype='dashed') +
  ylab('Sp. Richness Relative to Lockdown\n') +
  xlab('\n Days to Lockdown') +
