...
ebird19 <- filter(ebird_full, year == 2019 & date <= '2019-04-20' & date >= '2019-03-03') 
...
stats <- ebird20 %>%
  ...
  pivot_longer(cols=contains('Num.'), names_to='key', values_to='value')
...
# All Cities
stats <- ebird19 %>%
  select_sample(lockdown='2019-03-26') %>%
  distinct(trip_id, .keep_all = T) %>%
  dplyr::select(date, trip_id, duration, s_richness, 
                county, temperature, rain, hotspot_km) %>%
  mutate(prepost = if_else(date <= '2019-03-26', 'Before', 'After')) %>%
  group_by(prepost) %>%
  summarize(`Mean Trip Length` = round(mean(duration, na.rm=T), 2),
            `Mean Species Richness` = round(mean(s_richness, na.rm=T), 2),
            `Median Species Richness` = median(s_richness, na.rm=T),
...
# Top 20
stats <- ebird19 %>%
  select_sample(num_cities=20, lockdown='2019-03-26') %>%
  distinct(trip_id, .keep_all = T) %>%
  dplyr::select(date, trip_id, duration, s_richness, 
                county, temperature, rain, hotspot_km) %>%
  mutate(prepost = if_else(date <= '2019-03-26', 'Before', 'After')) %>%
  ...
  pivot_longer(starts_with('M'), names_to = ' ') %>%
  pivot_wider(names_from = prepost, values_from = value)
panel2 <- stargazer(stats, summary=F, digits=2, rownames=F)

star_panel(panel1, panel2, 
           panel.names = c('All Cities', 'Top 20'), 
           reg = F,
           panel.label.fontface = 'bold') %>%
  star_tex_write(file = './tables/t2_activity_trip_2019.tex')
#---------------------------------------------------
...
dd_type <- sample %>%
  ...
  summarize(`Num. Species/Trip` = mean(s_richness, na.rm=T),
            `B. Num. of Trips in 2020 Rel. to 2019` = n_distinct(trip_id),
            `A. Num. Active Users in 2020 Rel. to 2019` = n_distinct(observer_id)) %>%
  pivot_longer(cols=contains('Num.'), names_to='var', values_to='value') %>%
...
#-----------------------------------------------
sample <- did_placebo(ebird_full, num_cities=20)
dd_type <- sample %>%
  group_by(dif, year, protocol) %>%
  summarize(`Num. Species/Trip` = mean(s_richness, na.rm=T),
            `B. Num. of Trips in 2019 Rel. to 2018` = n_distinct(trip_id),
            `A. Num. Active Users in 2019 Rel. to 2018` = n_distinct(observer_id)) %>%
  pivot_longer(cols=contains('Num.'), names_to='var', values_to='value') %>%
  pivot_wider(names_from = c(year, protocol), values_from=value) %>%
...
# DD - Placebo - Species Richness
ggplot(dd_type[dd_type$var == 'Num. Species/Trip',], aes(x=(dif), y=value, group=key)) +
  geom_line(aes(color=key), size=1.5) + 
  labs(y='Difference in 2019 Species\nRichness per Trip Rel. to 2018\n',
       x='Days since 4th Wednesday in March', color = 'Trip Protocol') +
  geom_vline(xintercept=0, linetype='dashed') +
  geom_hline(yintercept=0, linetype='dashed') +
  scale_colour_viridis_d() +
  ...
  theme(panel.grid.minor = element_blank(), 
        axis.line = element_blank(),
        text = element_text(size=20))
ggsave('./figs/dd_sr_placebo_t20.png', height=5,width=15)
...
ggplot(dd_type[dd_type$var != 'Num. Species/Trip',], aes(x=(dif), y=value, group=key)) +
  ...
  theme(panel.grid.minor = element_blank(), 
        ...
        strip.text = element_text(size=15)) +
  facet_wrap(~var, scales='free')
ggsave('./figs/dd_activity_placebo_t20.png', height=6,width=15)
...
spec_cols <- c("Participation Constraint", "Fixed Effects")
estimates <- estimates %>% 
...
coef_plot <- ggplot(estimates, aes(x = spec_no, y = est)) + 
  ...
  geom_hline(yintercept=0, linetype='dashed') +
  geom_text(aes(label=round(est, 2)), nudge_x=0.15) +
...
combined_plot_main <- plot_grid(plotlist = c(list(coef_plot), spec_plots), 
                           ...
                           rel_heights = c(4, 1, 0.7),
                           ncol = 1,
                           align = "v")
save_plot("./figs/spec_curve_main.png", combined_plot_main)

#------------------------------------------------------------
# TABLE 4 : DD REGRESSION RESULTS - ROBUSTNESS
#------------------------------------------------------------
estimates <- data.frame()
...
# Protocol-Year FEs
# sample <- did(ebird_full, num_cities=20)
# estimates <- rbind(estimates, tidy(lm_robust(s_richness ~ TreatPost + Post + 
#                                                duration + rain + temperature + weekend + 
#                                                num_observers + hotspot_km + as.factor(protocol):year,
#                                              data = sample,
#                                              fixed_effects = county + as.factor(hour),
#                                              se_type = 'stata')) %>%
#                      filter(term == 'TreatPost') %>%
#                      select(estimate, std.error) %>%
#                      rename(est = 'estimate', se = 'std.error') %>%
...
combined_plot_rb <- plot_grid(plotlist = c(list(coef_plot), spec_plots), 
                           ...
                           label_fontface = "italic", 
                           vjust = 0.5, hjust = -0.1,
                           rel_heights = c(5,1.2,1.2,1.2,1.2,1.2), 
                           ncol = 1,
                           align = "v")
save_plot("./figs/spec_curve_rb.png", combined_plot_rb)

#------------------------------------------------------------
# TABLE 5 : DYNAMIC DD REGRESSION RESULTS
#------------------------------------------------------------
...
dd_time <- rbind(dd_time, tidy(lm_robust(s_richness ~ Treatment:bin + Treatment + bin + 
                                           duration + rain + temperature + weekend + 
                                           num_observers + hotspot_km + as.factor(county):date,
                                         data = sample,
                                         fixed_effects = county + protocol + as.factor(hour),
                                         se_type = 'stata')) %>%
...
#------------------------------------------------------------
# TABLE 6 : CITY LEVEL DD REGRESSION RESULTS
#------------------------------------------------------------
estimates <- data.frame()

# Top 20, 2 Trips
sample <- did_city(ebird_full, num_cities=20)
estimates <- rbind(estimates, tidy(lm_robust(s_richness ~ TreatPost + Treatment + Post + 
                                               duration + rain + temperature + protocol +
                                               num_observers + hotspot_km + weekend,
                                             data = sample,
                                             weights=n_trips,
...
# Top 20, 5 Trips
sample <- did_city(ebird_full, before=5, after=5, num_cities=20)
estimates <- rbind(estimates, tidy(lm_robust(s_richness ~ TreatPost + Treatment + Post + 
                                               duration + rain + temperature + protocol +
                                               num_observers + hotspot_km + weekend,
                                             data = sample,
...
# Top 20, 10 Trips
sample <- did_city(ebird_full, before=10, after=10, num_cities=20)
estimates <- rbind(estimates, tidy(lm_robust(s_richness ~ TreatPost + Treatment + Post + 
                                               duration + rain + temperature + protocol +
                                               num_observers + hotspot_km + weekend,
                                             data = sample,
...
combined_plot_main <- plot_grid(plotlist = c(list(coef_plot), spec_plots), 
                                ...
                                ncol = 1,
                                align = "v")
save_plot("./figs/spec_curve_city.png", combined_plot_main)
