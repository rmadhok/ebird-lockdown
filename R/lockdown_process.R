...
# Stationary trips 
ebd_use <- filter(ebird, protocol_type %in% c('Stationary','Traveling')) # n=2,181,612

# Complete checklists (# n = 2,127,772)
ebd_use <- filter(ebd_use, all_species_reported == 1)

# Drop Duplicates from group trips (n=1,628,734)
...
# Duration b/w 5-240 mins (see Callaghan et al. 2019) n=1,499,975
...
# Drop if checklist has 1 bird (see Walker & Tayler, 2017) n=1,494,090
...
# drop sightings outside map (n=1,491,372)
...
ebd_use <- left_join(ebd_use, ebd_trip[,c('trip_id','hotspot_km')],
                     by='trip_id')
