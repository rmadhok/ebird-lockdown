...
m_sp <- sp_dist %>%
  ...
  mutate(keep = ifelse(YEAR == 2020 & prepost=='POST' & n_days >= 7, 1,0), # dummy if species observed >= 7 times post-lockdown 
         keep = max(keep)) %>%
...
#------------------------------------
# MARGINAL SPECIES STATISTICS
#------------------------------------
rm(list=c('sp_main', 'sp_apx', 'sp'))

# % of Checklists reporting each marginal species
...
n_trips <- sample %>%
  group_by(COUNTY, YEAR) %>%
  summarize(n_trips = n())
sp_dist <- merge(sp_dist, n_trips, by=c('COUNTY', 'YEAR'))
...
m_iucn <- left_join(m_stats, iucn[, c('sp_clean', 'iucn')], by='sp_clean') %>%
  select(-sp_clean) %>%
  mutate(iucn = replace(iucn, species=='Asian Koel', 'LC'),
